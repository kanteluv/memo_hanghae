...
//    Set<Memo> findAllMemoAndComments();
    ...
    @Query("select new com.sparta.hanghaemamo.dto.MemoCommentResponseDto(m.id, m.username, m.contents, m.contentName, c) from Memo m left join Comment c on m.id = c.memo.id ORDER BY m.modifiedAt DESC")
    List<MemoCommentResponseDto> findCommentsAndMemos();
...
//    Set<Memo> findMemoAndComments(@Param("memoId") Long memoId);
    ...
    @Query("select new com.sparta.hanghaemamo.dto.MemoCommentResponseDto(m.id, m.username, m.contents, m.contentName, c) from Memo m left Join Comment c on m.id = c.memo.id WHERE m.id = :memoId ORDER BY m.modifiedAt DESC")
